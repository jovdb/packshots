...
export class ConeGeometry implements IGeometry {
  ...
  public intersect(cameraPosition: Vector3, rayDirection: Vector3) {
    ...
    const b = new Vector3(rayDirection.x, rayDirection.y, A).dot(new Vector3(cameraPosition.x, cameraPosition.y, -B))
      * 2;
    const c = new Vector3(cameraPosition.x, cameraPosition.y, B).dot(
      new Vector3(cameraPosition.x, cameraPosition.y, -B),
    );
    ...
    const q = cameraPosition.clone().add(rayDirection.multiplyScalar(t)); // mutates rayDiraction!
    const y = q.z;
    ...
    if (Math.abs(y) > hh) return undefined;
